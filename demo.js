...
async function main() {
	const url =
		`mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.jjmb1.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
	console.log(url);
...
async function findListingsWithMinimumBedroomsAndBathroomsAndMostRecentReviews(
	client,
	{
	...
	if (results.length > 0) {
		console.log(
			`Found ${results.length} listings with ${minimumNumberOfBedrooms} bedrooms and ${minimumNumberOfBathrooms} bathrooms`
		);
		...
		console.log(
			`listings with ${minimumNumberOfBedrooms} bedrooms and ${minimumNumberOfBathrooms} bathrooms`
		);
	}
}
